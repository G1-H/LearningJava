    ...
    public static void main(String[] args) {
        ...
        for(Order order: orders){
            try {
                ...
                if (order.getStatus().equals(OrderStatus.PROCESSING)) throw new RuntimeException(customer + "님의 주문이 아직 주문 처리 중입니다.");
                List<OrderItem> orderItemArrayList = order.getOrderItems().orElseThrow(()->{throw new RuntimeException(order.getCustomer() +"님의 주문 아이템들이 누락 되었습니다.");});
                totalOrderCount++;
